      ...
      <header className={styles.header} style={{ '--links': links.length }}>
        <nav className={`${styles.menu} ${links.length > 0 ? styles.hasNav : ''}`}>
          {links.length > 0 ? links.map(({ id, title }) => (
            <Link href={`#${id}`} key={id} className={id === activeId ? styles.isActive : ''} onClick={e => scrollToLink(e, id)}>{title}</Link>
          ))
            : <Link href="/">Sustainability Toolkit</Link>}
        </nav>
        <Link href="/principles" className={`${styles.principles} ${router.pathname === '/principles' ? styles.isActive : ''}`}>Goals & Principles</Link>
